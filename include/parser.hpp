/*
 * from here:
 * https://github.com/RishabhRD/libparse/blob/master/include/parser/parser.hpp
 */
...
concept Parser = std::regular_invocable<ParserFunc, std::string_view> &&
    std::same_as<std::invoke_result_t<ParserFunc, std::string_view>,
                 parsed_t<typename std::invoke_result_t<
                     ParserFunc, std::string_view>::value_type::first_type>>;
...
namespace detail {
  ...
  template <typename F>
  constexpr auto make_pipe_adapter(F &&f) {
  ...
  auto constexpr transform(P && p, F && f) noexcept {
    ...
    return [p = std::forward<P>(p),
      ...
      return std::make_pair(std::invoke(f, std::move(opt_i_res->first)),
                            opt_i_res->second);
    };
  }
  ...
  or_with(P1 && p1, P2 && p2) noexcept {
    ...
    return [p1 = std::forward<P1>(p1),
            p2 = std::forward<P2>(p2)](std::string_view str) -> R {
  ...
  constexpr auto combine(P1 && p1, P2 && p2, F && f) noexcept {
    using R = parsed_t<
        std::invoke_result_t<F, parser_value_t<P1>, parser_value_t<P2>>>;
  ...
  then(P && p, F && f) {
    ...
    return [p = std::forward<P>(p),
      ...
      auto opt_i_res = std::invoke(p, str);
      if (!opt_i_res) return {};
      return std::invoke(std::invoke(f, std::move(opt_i_res->first)),
                         opt_i_res->second);
    };
  }
  ...
  requires(std::same_as<std::invoke_result_t<F, B, parser_value_t<P>>,
                        B>) constexpr auto
  many(P && p, B b, F && f) {
    return [p = std::forward<P>(p), b = std::move(b),
  ...
  exactly_n(P && p, B b, F && f, std::size_t n) {
    return [p = std::forward<P>(p), b = std::move(b), f = std::forward<F>(f),
            n](std::string_view str) -> parsed_t<B> {
      auto init = b;
      for (size_t i{}; i < n; ++i) {
        if (auto res = p(str)) {
          init = std::invoke(f, std::move(init), std::move(res->first));
  ...
  seperated_by(P1 && p1, P2 && p2, B b, F && f) {
    return
        [p1 = std::forward<P1>(p1), p2 = std::forward<P2>(p2), b = std::move(b),
         f = std::forward<F>(f)](std::string_view str) -> parsed_t<B> {
          auto res = std::invoke(p1, str);
          if (!res) return {};
          return many(ignore_previous(p2, p1),
                      std::invoke(f, b, std::move(res->first)), f)(res->second);
        };
  }
...
template <typename F>
constexpr auto piped(F &&f) noexcept {
...
constexpr auto combine_with =
    piped([]<typename P1, typename P2, typename F>(P1 &&p1, P2 &&p2, F &&f) {
      return detail::combine(std::forward<P1>(p1), std::forward<P2>(p2),
                             std::forward<F>(f));
    });

constexpr auto ignore_previous = piped([]<typename P1, typename P2>(
                                           P1 &&p1, P2 &&p2) noexcept {
...
template <typename F>
constexpr auto if_char_satisfies(F &&f) {
...
constexpr auto many1 = piped([]<typename P, typename B, typename F>(P &&p, B b,
                                                                    F &&f) {
...
constexpr auto exactly_n =
    piped([]<typename P, typename B, typename F>(P &&p, B b, F &&f, size_t n) {
      return detail::exactly_n(std::forward<P>(p), std::move(b),
                               std::forward<F>(f), n);
    });

constexpr auto seperated_by =
    piped([]<typename P1, typename P2, typename B, typename F>(P1 &&p1, P2 &&p2,
                                                               B b, F &&f) {
      return detail::seperated_by(std::forward<P1>(p1), std::forward<P2>(p2),
                                  std::move(b), std::forward<F>(f));
    });
...
template <std::predicate<char> Predicate>
constexpr auto many_if(Predicate &&p) {
  using namespace std::string_view_literals;
